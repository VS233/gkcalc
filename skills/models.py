...
class SkillSection(models.TextChoices):
    ATTACK     = 'attack',     'Атака'
...
class SkillNode(models.Model):
    """Узел дерева навыков."""
    section     = models.CharField(max_length=16, choices=SkillSection.choices, verbose_name='Раздел')
    branch      = models.CharField(max_length=4,  choices=SkillBranch.choices,  verbose_name='Ветка')
    ...
    def __str__(self):
        return f'[{self.get_section_display()} / ветка {self.branch}] {self.name} (ряд {self.order})'


class SkillEdge(models.Model):
    """Связь между узлами — вертикальная или диагональная, направление только вниз."""
...
class SkillBonus(models.Model):
    """Бонус к стату за вложенные очки в узел."""
